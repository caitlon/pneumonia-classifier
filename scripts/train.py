...
from pneumonia_classifier.utils import format_metrics, get_device, save_model
...
class MLflowLogger:
    ...
    def log_to_mlflow(self, run_id: str) -> None:
        ...
        if self.log_buffer:
            ...
            epoch_tag = (
                f"_epoch_{self.current_epoch}" if self.current_epoch is not None else ""
            )
...
def train_model(
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        # Log tags
        mlflow.set_tags(
            {"task": "classification", "domain": "medical", "dataset": "pneumonia-xray"}
        )
...
def main():
    ...
    parser.add_argument(
        "--run_name", type=str, default=None, help="Run name for MLflow"
    )
    parser.add_argument(
        "--patience",
        type=int,
