    ...
    try:
        ...
        if not registry.admin_user_enabled:
            ...
            registry = acr_client.registries.update(
                resource_group, registry_name, {"admin_user_enabled": True}
            )
    except Exception:
        ...
        registry = acr_client.registries.begin_create(
            resource_group,
            registry_name,
            {"location": location, "sku": {"name": sku}, "admin_user_enabled": True},
        ).result()
...
def build_and_push_image(
    ...
    acr_info = ensure_acr_exists(
        credential, subscription_id, resource_group, registry_name, location
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--client-id", type=str, help="Azure service principal client ID"
    )
    parser.add_argument(
        "--client-secret", type=str, help="Azure service principal client secret"
    )
    ...
    logger.info(
        f"python scripts/deploy_to_azure.py --subscription-id {args.subscription_id} "
