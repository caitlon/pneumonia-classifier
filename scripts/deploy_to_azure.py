...
from pneumonia_classifier.config import TRAINING_CONFIG, MODEL_CONFIG
...
def deploy_to_azure(
    ...
    container_group = ContainerGroup(
        ...
        ] if registry_password else None
    )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Deploy pneumonia classification model to Azure")
    ...
    parser.add_argument("--container-name", type=str, default="pneumonia-api",
                        help="Container instance name")
    parser.add_argument("--dns-name", type=str, default="pneumonia-classifier",
    ...
    parser.add_argument("--registry-username", type=str,
                        help="ACR username (defaults to registry name)")
    parser.add_argument("--registry-password", type=str,
                        help="ACR password (required if using ACR)")
    ...
    # Deploy to Azure
    deployment_result = deploy_to_azure(
    ...
    logger.info(f"API is now available at: http://{deployment_result['fqdn']}:8000")
    logger.info("You can test it with: python scripts/predict.py --image_path <path> " + 
                f"--api_url http://{deployment_result['fqdn']}:8000")
